...
const totalStock = computed(() => filteredMedicines.value.reduce((acc, m) => acc + Number(m.stock_qty), 0));
...
function exportCSV() {
  ...
  rows.push([]);
  rows.push(["", "", "", totalStock.value, "", "", totalUnitsSold.value, totalRevenue.value, totalCost.value, totalProfit.value, avgProfitMargin.value]);
